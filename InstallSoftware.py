...
GitLocations    = [ "https://github.com/HU-TI-DEV/bmptk.git",
...
logger.info("Build GIT individual file download locations")
...
for repo in GitLocations:
    ...
    if os.path.exists(repoName):
        ...
        if repoName == "Catch2":
            logger.info("adjusting Catch2" )
            os.chdir(repoName)
            os.system('git checkout' + " v2.x")
            os.chdir("..")

# download any git files individually
...
for ex_folder in example_folders:
    open(('./%s/CMakeLists.txt' % ex_folder), 'w').write(voorbeeld_cmake_contents.replace("your-project-name", ex_folder))

# done


logger.info("Installation completed")
logger.info("\n*****   Please execute pathupdate.bat in an admin rights CMD and reboot the computer to make the PATH change effective!! *****")
